    ...
    public static class BasicTextReaderExtensions
    {
        ...
        public static bool TryReadIntegerOrFloatNumber(this TextReader inputStream, StringBuilder text, out Token token)
        {
            ...
            if (inputStream.TakeIf(char.IsDigit, text))
            {
                ...
                {
                    if (!inputStream.TakeIf(char.IsDigit, text))
                        throw new UnexpectedCharacterException((char)inputStream.Peek());
        ...
        private static readonly IDictionary<string, Token> keywords = new SortedDictionary<string, Token>(StringComparer.InvariantCultureIgnoreCase)
        {
            { "and", Token.And },
            { "dim", Token.Dim },
            { "else", Token.Else },
